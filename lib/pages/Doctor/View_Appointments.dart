...
class View_Appointments extends StatefulWidget {
  ...
  @override
  State<View_Appointments> createState() => _View_AppointmentsState();
}
...
class _View_AppointmentsState extends State<View_Appointments> {
  ...
  Future<bool> getAppointments() async {
    print(widget.doc_ID);
    appointment =
        await remort_services().getAllAppointments_byDoctor(widget.doc_ID);
    if (appointment != null) {
      setState(() {
        IsLoaded = true;
      });
                        ...
                        SlidableAction(
                          ...
                          onPressed: (context) {
                            if (appointment![index].isapproved != 0) {
                              try {
                                remort_services().UnApprove_Appointment(
                                    0, appointment![index].id);
                                setState(() {
                                  getAppointments();
                                });
                                ScaffoldMessenger.of(context).showSnackBar(
                              ...
                              ScaffoldMessenger.of(context).showSnackBar(
                                  const SnackBar(
                                      content: Text('Already Un-Approved!')));
                            }
                          },
                          backgroundColor: Colors.white,
                          foregroundColor: Colors.blue,
